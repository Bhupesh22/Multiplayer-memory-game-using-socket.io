...
import DefaultMemoryGameSettings from './games/DefaultMemoryGameSettings';
import GameAreaFactory from './games/GameAreaFactory';
import InteractableArea from './InteractableArea';
import Leaderboard from './Leaderboard';
import LeaderboardDatabase from './LeaderboardDatabase';
import ViewingArea from './ViewingArea';
import MemoryGameGameArea from './games/MemoryGameGameArea';
...
export default class Town {
  ...
  get defaultMemoryGameSettings(): DefaultMemoryGameSettings {
    return this._defaultMemoryGameSettings;
  }
  ...
  private _defaultMemoryGameSettings: DefaultMemoryGameSettings;
  ...
  constructor(
    ...
    this._leaderboardDatabase = new LeaderboardDatabase();
    this._defaultMemoryGameSettings = new DefaultMemoryGameSettings();
  }
  ...
  async addPlayer(userName: string, socket: CoveyTownSocket): Promise<Player> {
    ...
    socket.on('memoryGameSettingsUpdate', (settings: MemoryGameSettingsCommand) => {
      if (newPlayer.isAdmin) {
        this._defaultMemoryGameSettings = { ...settings.settings };
        this._interactables
          .filter(eachInteractable => eachInteractable instanceof MemoryGameGameArea)
          .forEach(eachInteractable => {
            (eachInteractable as MemoryGameGameArea).defaultMemoryGameSettings =
              this._defaultMemoryGameSettings;
          });
        socket.emit('memoryGameSettingsResponse', {
    ...
    const gameAreas = objectLayer.objects
      ...
      .map(eachGameAreaObj =>
        GameAreaFactory(
          ...
          this._defaultMemoryGameSettings,
          this._leaderboardDatabase,
        ),
      );

    const leaderboardAreas = objectLayer.objects
      .filter(eachObject => eachObject.type === 'LeaderboardArea')
      .map(eachLeaderboardObj =>
