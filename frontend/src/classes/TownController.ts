...
import {
  ...
  InteractableID,
  MemoryGameSettings,
  MemoryGameSettingsCommand,
  MemoryGameSettingsCommandResponse,
  PlayerID,
  PlayerLocation,
  TownSettingsUpdate,
...
import MemoryGameAreaController from './interactable/MemoryGameAreaController';
...
export default class TownController extends (EventEmitter as new () => TypedEmitter<TownEvents>) {
  ...
  public get leaderboardAreas() {
    const ret = this._interactableControllers.filter(
      eachInteractable => eachInteractable instanceof LeaderboardAreaController,
    );
    return ret as LeaderboardAreaController[];
  }

  public get adminController() {
  ...
  /**
   * Sends an memory game settings command to the townService. Returns a promise that resolves
   * when the command is acknowledged by the server.
   *
   * If the command is not acknowledged within SOCKET_COMMAND_TIMEOUT_MS, the promise will reject.
   *
   ...
   * If the command is acknowledged with an error, the promise will reject with the error.
   *
   * @param newSettings The new settings for the default memory game
   * @returns A promise that resolves if the command succeeded, and rejects if not
   *
   **/
  public async sendMemoryGameSettingsCommand(newSettings: MemoryGameSettings): Promise<void> {
    return new Promise((resolve, reject) => {
      const watchdog = setTimeout(() => {
        reject('Command timed out');
      }, SOCKET_COMMAND_TIMEOUT_MS);

      const commandMessage: MemoryGameSettingsCommand = {
        commandID: nanoid(),
        settings: newSettings,
      };

      const ackListener = (response: MemoryGameSettingsCommandResponse) => {
        if (response.commandID === commandMessage.commandID) {
          clearTimeout(watchdog);
          this._socket.off('memoryGameSettingsResponse', ackListener);
          if (response.success) {
            resolve();
          } else {
            reject();
      ...
      this._socket.on('memoryGameSettingsResponse', ackListener);
      this._socket.emit('memoryGameSettingsUpdate', commandMessage);
    });
  }
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      this._socket.on('initialize', initialData => {
        ...
        initialData.interactables.forEach(eachInteractable => {
          ...
          } else if (isMemoryGameArea(eachInteractable)) {
            this._interactableControllers.push(
              new MemoryGameAreaController(eachInteractable.id, eachInteractable, this),
            );
          } else if (isLeaderboardArea(eachInteractable)) {
            const leaderboardAreaController = new LeaderboardAreaController(
              eachInteractable.id,
              eachInteractable,
              this,
            );
            if (initialData.memoryGameSettings) {
              this._adminController = new AdminController(
                this,
                leaderboardAreaController,
                initialData.memoryGameSettings,
              );
            } else {
              throw new Error('Memory game settings not found');
            }
            this._interactableControllers.push(leaderboardAreaController);
          }
        });
...
export function useInteractableAreaController<T>(interactableAreaID: string): T {
  const townController = useTownController();
  const interactableAreaControllerGameCheck = townController.gameAreas.find(
    eachArea => eachArea.id == interactableAreaID,
  );
  const interactableAreaControllerLeaderboardCheck = townController.leaderboardAreas.find(
    eachArea => eachArea.id == interactableAreaID,
  );
  if (interactableAreaControllerGameCheck) {
    return interactableAreaControllerGameCheck as unknown as T;
  } else if (interactableAreaControllerLeaderboardCheck) {
    return interactableAreaControllerLeaderboardCheck as unknown as T;
  } else {
...
/**
 * A react hook to retrieve the admin controller
 *
 *
 * This hook relies on the TownControllerContext.
 *
...
export function useAdminController(): AdminController {
  const townController = useTownController();
  assert(townController.adminController);
  return townController.adminController;
}
